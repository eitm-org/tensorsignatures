...
class TensorSignature(object):
    ...
    def L1ij(self):
        ...
        if self.objective == 'nbconst':
            ...
            self._L1ij = self.tau \
                * tf.math.log(self.tau) \
                - tf.math.lgamma(self.tau) \
                + tf.math.lgamma(self.C1 + self.tau) \
                + self.C1 * tf.math.log(self.Chat1) \
                - tf.math.log(self.Chat1 + self.tau) \
                * (self.tau + self.C1) \
                - tf.math.lgamma(self.C1 + 1)
        if self.objective == 'poisson':
            if self.verbose:
                print('Using poisson likelihood')
            self._L1ij = self.C1 \
                * tf.math.log(self.Chat1) \
                - self.Chat1 \
                - tf.math.lgamma(self.C1 + 1)
    ...
    def L2ij(self):
        ...
        if self.objective == 'nbconst':
            self._L2ij = self.tau \
                * tf.math.log(self.tau) \
                - tf.math.lgamma(self.tau) \
                + tf.math.lgamma(self.C2 + self.tau) \
                + self.C2 * tf.math.log(self.Chat2) \
                - tf.math.log(self.Chat2 + self.tau) \
                * (self.tau + self.C2) \
                - tf.math.lgamma(self.C2 + 1)
        if self.objective == 'poisson':
            self._L2ij = self.C2 \
                * tf.math.log(self.Chat2) \
                - self.Chat2 \
                - tf.math.lgamma(self.C2 + 1)
        return self._L2ij
...
class TensorSignatures2D(TensorSignature):
    ...
    def S(self):
        ...
        """
        #sig = tf.Variable(tf.random.truncated_normal([dim-1, self.r], dtype=self.dtype))
        #sig = tf.nn.softmax(tf.concat([sig, tf.zeros([1, self.r], dtype=self.dtype)], axis=0), dim=0)

        self.S0 = tf.Variable(tf.random.truncated_normal(
            [self.p-1, self.rank], dtype=self.dtype, seed=self.seed), name='S0')
        self._S = tf.nn.softmax(
            tf.concat([self.S0, tf.zeros([1, self.rank])], axis=0), aixs=0, name='S1')
        if self.verbose:
